...
export async function GET(
  ...
  try {
    ...
    const rows = await query('SELECT * FROM product_banner_ads WHERE id = ?', [id]);

    const bannerAds = rows as any[];
    if (bannerAds.length === 0) {
      return NextResponse.json({ error: 'Product banner ad not found' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    const updatedRows = await query('SELECT * FROM product_banner_ads WHERE id = ?', [id]);
    const updatedBannerAds = updatedRows as any[];
    if (updatedBannerAds.length === 0) {
      return NextResponse.json({ error: 'Product banner ad not found after update' }, { status: 404 });
    }

    return NextResponse.json(updatedBannerAds[0]);
  } catch (error) {
    console.error('Error updating product banner ad:', error);
    return NextResponse.json({ error: 'Failed to update product banner ad' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await query('DELETE FROM product_banner_ads WHERE id = ?', [id]);
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Error deleting product banner ad:', error);
    return NextResponse.json({ error: 'Failed to delete product banner ad' }, { status: 500 });
  }
}
